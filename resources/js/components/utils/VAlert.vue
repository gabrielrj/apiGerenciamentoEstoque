        ...
        abreMensagens(mensagens){
            ...
            if(resposta != null && resposta != undefined){
                if(typeof resposta.data.errors != "undefined") {
                    ...
                    this.listaDeMensagens.push({
                        id: this.listaDeMensagens.length + 1,
                        mensagem: (resposta.data.msg != undefined && resposta.data.msg != null) ? resposta.data.msg : (resposta.data.message != undefined && resposta.data.message != null ? resposta.data.message : 'Ocorreu um erro inesperado, informe ao administrador do sistema.'),
                        tipo_mensagens: resposta.status < 200 || resposta.status > 299 ? 'error' : 'success'
                    })
                }else{
                    this.listaDeMensagens.push({
                        id: this.listaDeMensagens.length + 1,
                        mensagem: 'Ocorreu um erro inesperado, informe ao administrador do sistema.',
                        tipo_mensagens: 'error'
                    })
                }
